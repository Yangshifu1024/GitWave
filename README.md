...
- **Workspace management** — multiple workspaces, repo tabs with drag-reorder, adding several local repositories at once (already-added paths and repeats inside the batch are skipped, unusable paths are reported without failing the batch), per-workspace AI context; a workspace is an abstraction, not a directory
...
- **Advanced Git** — stash (scoped to the active repo), interactive rebase, worktrees, submodules, LFS, remotes, .gitignore editor, Git hooks panel, repo health checks
