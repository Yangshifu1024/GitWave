...
export function CommitExplainModal({
  ...
  const copy = async () => {
    try {
      ...
      setStatus(t("commits.explain.copied"));
    } catch {
      setStatus(t("commits.explain.clipboardUnavailable"), "danger");
    }
  };

  const title = subject
    ? t("commits.explain.titleWithSubject", { subject })
    : t("commits.explain.titleWithSha", { sha: sha.slice(0, 7) });
  ...
  return (
    <Modal
      ...
      description={t("commits.explain.description")}
      size="lg"
      footer={
        <>
    ...
    >
      <div className="flex flex-col gap-3">
        {genMut.isPending ? (
          <p className="text-xs text-text-muted">{t("commits.explain.thinking")}</p>
        ) : error ? (
